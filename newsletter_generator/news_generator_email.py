...
def create_highlights_dict(df, index):
    ...
    highlight_config_dict = {
        ...
        "HIGHLIGHT_IMAGE": df.at[index, "Link_to_pic"].replace(
            "open?id", "uc?export=view&id"
        ),
...
def create_header(header_config):
    ...
    """
    with open("header.html", "r", encoding="utf-8") as template, open(
        "newsletter_header.html", "w", encoding="utf-8"
    ) as newsletter:
...
def create_highlight(highlight_config, car_specs):
    """
    Creates the newsletter highlight section from a template 
    ...
    """
    with open("highlights.html", "r", encoding="utf-8") as template, open(
        "newsletter_highlight.html", "w", encoding="utf-8"
    ) as newsletter:
        for line in template:
...
def create_content(content_dict, content_car_specs):
    """
    Creates the newsletter content section from a template 
    and replaces placeholders with actual values.
    ...
    """
    with open("content.html", "r", encoding="utf-8") as template, open(
        "newsletter_content.html", "a", encoding="utf-8"
    ) as newsletter:
        for line in template:
            ...
            for line in template:
                for reg in newsletter_regex:
                    if newsletter_regex[0] in line:
                        header_file = open(
                            "newsletter_header.html", "r", encoding="utf-8"
                        )
                    ...
                    if newsletter_regex[1] in line:
                        highlight_file = open(
                            "newsletter_highlight.html", "r", encoding="utf-8"
                        )
                    ...
                    if newsletter_regex[2] in line:
                        highlight_file = open(
                            "newsletter_content.html", "r", encoding="utf-8"
                        )
                        highlight_data = highlight_file.read()
                        highlight_file.close()
                        line = line.replace(reg, highlight_data)
...
def parse_excel_sheets(excel_file):
    ...
    try:
        ...
        logging.error("An error occurred while parsing the Excel file: %s", e)
        raise
...
def generate_newsletter():
    ...
    try:
        ...
        logging.error("An error occurred while opening the Excel file: %s", e)
        raise
    ...
    if not pd.notna(newsletter_date):
        day_name = time.strftime('%A')
        day = time.strftime('%d')
        month = time.strftime('%B')
        year = time.strftime('%Y')
        newsletter_date = f"{day_name},  {day} de {month}  {year}"
    ...
    cars_df = (
        cars_df[cars_df["Ativo"].astype(str) != "0"]
        .sort_values(by=["Display_no"])
    ...
    for index in cars_df.index[1:]:
        create_content(
            create_content_dict(cars_df, index), create_car_specs(cars_df, index)
        )
...
def main():
    try:
        ...
        logging.error("Oops! Something went wrong: %s", e)
